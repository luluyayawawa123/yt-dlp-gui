...
- 备用方式：如需 ZIP，可下载 `YT-DLP-GUI-mac-v*.zip`，解压后将“YT-DLP GUI”拖入“应用程序（Applications）”文件夹；多数电脑默认不显示扩展名，看不到“.app”是正常的

3) 首次运行（安全与权限）
- 由于来自未公证开发者，首次运行可能被系统拦截：
  - 打开“系统设置（System Settings）” → “隐私与安全性（Privacy & Security）” → 在“安全性”中找到被拦截的“YT-DLP GUI”，点击“仍要打开（Open Anyway）”
...
基础模式（推荐）
1. 打开应用，在“浏览器”处选择 cookies 来源：Safari（默认）/Chrome/Edge
   - 必须先在所选浏览器中登录 YouTube 账号（下载会员/受限视频需要登录）
2. 将一个或多个 YouTube 视频/播放列表/频道链接粘贴到输入框（每行一个）
3. 选择下载目录
4. 画质选择：保持“最高画质”即可下载可用的最高分辨率（可达 4K/8K）；若仅需音频请选择“MP3”
5. 如需字幕，勾选“下载字幕”（自动转换为 SRT）
6. 点击“开始下载”，在下方查看实时进度；下载完成后可一键打开文件或所在文件夹
